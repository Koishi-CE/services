...
export default defineConfig({
    ...
    deps: {
        // 依赖全部 external（@koishi-ce 框架为 peer 单实例，puppeteer-core /
        // puppeteer-finder 为运行时 dependency），不打进产物
        bundle: false,
        dts: {
            // koishi 生态 d.ts 含 namespace 成员 re-export，dts 打包无法解析，
            // puppeteer-core 类型面庞大且自带 exports 边界，均保持外部引用
            // （消费端由 peer / dependency 提供）
            neverBundle: [/^@koishi-ce/, /^@satorijs\//, /^cordis/, /^minato/, /^cosmokit/, /^puppeteer/],
        },
    },
});
